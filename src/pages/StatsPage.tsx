    ...
    <ResponsiveContainer width="100%" height={230}>
      <ComposedChart data={chartData} margin={{ top: 12, right: 8, left: 4, bottom: 0 }}>
        ...
        <Area
          ...
          stroke={lineColor}
          strokeWidth={2}
          fill={isPositive ? 'url(#positiveGrad)' : 'url(#negativeGrad)'}
          dot={false}
...
export default function StatsPage() {
  ...
  const loadStats = useCallback(async () => {
    ...
    const { data: players } = await supabase
      ...
      .select('result, total_buyin, cashout, position, sessions(status, ended_at)')
      .eq('user_id', user.id)
      .not('result', 'is', null);

    type SessionRef = { status: string; ended_at: string | null } | null;

    const closed: SessionData[] = (players ?? [])
      .map((p) => ({ ...p, _sessions: p.sessions as unknown as SessionRef }))
      .filter((p) => p._sessions?.status === 'closed' && p._sessions?.ended_at)
      .map((p) => ({
        result: Number(p.result),
        total_buyin: Number(p.total_buyin),
        cashout: Number(p.cashout ?? 0),
        position: p.position as number | null,
        date: p._sessions!.ended_at!,
      }))
  ...
  return (
    <Box sx={{ px: 2, pt: 3, pb: 6, maxWidth: 600, mx: 'auto' }}>
      {/* Header */}
      <Typography variant="h5" fontWeight={800} sx={{ mb: 0.25 }}>Your Stats</Typography>
          ...
          <CardContent>
            ...
            <Typography color="text.secondary">
              {allSessions.length === 0
                ? 'Play your first session to see stats.'
